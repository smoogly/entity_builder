...
describe("Typeorm set relation", function () {
    ...
    afterEach(async () => {
        if (downQueries === null) {
            throw new Error("Down queries not defined, did the test wait for setup to finish?");
        }
    ...
    it("Should throw if origin entity doesn't exist", async () => {
        ...
        try {
            ...
            expect(e.message).to.contain("do not exist");
        }
    });
    ...
    it("Should throw if entities don't have direct relation", async () => {
        ...
        try {
            await setRelation(
                ...
                [{
                    from: { type: Origin, id: String(originInstance.id) },
                    to: { type: Unrelated, id: String(unrelatedInstance.id) }
                }]
            );
            throw new Error("Expected to throw");
        } catch (e) {
